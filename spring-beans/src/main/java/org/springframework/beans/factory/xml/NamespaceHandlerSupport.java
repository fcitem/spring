...
public abstract class NamespaceHandlerSupport implements NamespaceHandler {
    ...
    /**
     * 存储以Element的name为键，以BeanDefinitionParser的实现为值的map<br>
    ...
    /**
     * 存储以Element的name为键，以BeanDefinitionDecorator的实现为值的map<br>
    ...
    /**
     * 存储以Attr的name为键，以BeanDefinitionDecorator的实现为值的map<br>
    ...
    public BeanDefinition parse(Element element, ParserContext parserContext) {
        //后面的parse方法根据扩展进行真正解析获得一个BeanDefinition
    ...
    private BeanDefinitionParser findParserForElement(Element element, ParserContext parserContext) {
        //寻找元素名称,也就是<myTag:user>中的user
        String localName = parserContext.getDelegate().getLocalName(element);
        //根据元素名称找到相应的解析器,也就是自定义扩展NameSpaceHandler中重写init方法时调用的registerBeanDefinitionParser方法注册进去的类
