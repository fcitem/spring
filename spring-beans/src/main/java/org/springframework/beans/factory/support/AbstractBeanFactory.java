...
public abstract class AbstractBeanFactory extends FactoryBeanRegistrySupport implements ConfigurableBeanFactory {
    ...
    protected <T> T doGetBean(
        ...
        // spring创建bean的原则是不等bean创建完成就会将创建bean的ObjectFactory提早曝光,一但下个bean创建时候需要依赖上个bean则直接使用ObjectFactory
        // 先从缓存中去取,处理已经被创建过的单例模式的bean，对这种bean的请求不需要重复地创建
        // 获取单例bean也叫共享bean
        ...
        if (sharedInstance != null && args == null) {
            ...
            //检测当前bean是否为factoryBean类型的bean,如果是则需要调用factoryBean实例的getObject作为返回值
            ...
            //如果自己加载的BeanDefinition中不包括相应name的,则从parentBeanFactory中查找
            ...
            try {
                //将前面解析BeanDefinition时候创建的GenericBeanDefinition转化为RootBeanDefinition,如果指定beanName是子bean的话同时会合并父类的相关属性
                final RootBeanDefinition mbd = getMergedLocalBeanDefinition(beanName);
                checkMergedBeanDefinition(mbd, beanName, args);

                // Guarantee initialization of beans that the current bean depends on.
                //从BeanDefinition的dependsOn属性获取
                String[] dependsOn = mbd.getDependsOn();
                //如果依赖存在则需要递归实例化依赖的bean
                if (dependsOn != null) {
                    for (String dep : dependsOn) {
                        ...
                        //缓存依赖dep-beanName;beanName->dep关系
        ...
        //检查需要的类型是否为bean的实际类型,若不是则进行相应的转换
    ...
    protected RootBeanDefinition getMergedBeanDefinition(
        ...
        synchronized (this.mergedBeanDefinitions) {
            ...
            if (mbd == null) {
                ...
                //如果beanDefinition里面没有scope相关的配置信息,则设置默认的scope域为SINGLETON
    ...
    protected Object getObjectForBeanInstance(
        ...
        //如果指定的类型是工厂相关(以&为前缀)且beanInstance又不是factoryBean类型则不通过
        ...
        //beanInstance可能是正常的bean或者factoryBean
        //beanInstance不是factoryBean的实例且满足factoryBean的name命名规则,说明是factoryBean只是没加载实例化,否则直接返回
        if (!(beanInstance instanceof FactoryBean) || BeanFactoryUtils.isFactoryDereference(name)) {
            return beanInstance;
        }
        //开始加载factoryBean
        ...
        if (object == null) {
            ...
            if (mbd == null && containsBeanDefinition(beanName)) {
                //将前面BeanDefinition解析时构建的GenericBeanDefinition转换为RootBeanDefinition,如果指定beanName是子bean的话同时会合并父类的相关属性
                mbd = getMergedLocalBeanDefinition(beanName);
            }
            //是否是用户自定义的而不是用户程序本身定义的
