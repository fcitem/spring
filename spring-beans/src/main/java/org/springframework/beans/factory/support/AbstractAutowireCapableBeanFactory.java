...
/**提供创建bean、自动注入、初始化以及应用bean的后处理器<br/>
...
public abstract class AbstractAutowireCapableBeanFactory extends AbstractBeanFactory
	 ...
	 */
	/**是否在循环引用的情况下注入原始bean实例,即使注入的bean最终要被包装*/
	...
	/** 缓存未完成的FactoryBean实例: FactoryBean name --> BeanWrapper */
	private final Map<String, BeanWrapper> factoryBeanInstanceCache =
			new ConcurrentHashMap<String, BeanWrapper>(16);

	/** Cache of filtered PropertyDescriptors: bean Class -> PropertyDescriptor array */
	/** 缓存被过滤掉的PropertyDescriptor */
	...
	/**忽略给定类型的接口依赖自动注入,典型应用是通过其他方式解析Application上下文注入依赖,类似于BeanFactory通过BeanFactoryAware进行注入<br/>
	...
	protected Object createBean(String beanName, RootBeanDefinition mbd, Object[] args) throws BeanCreationException {
		...
		try {
            ...
            //前置处理,给BeanPostProcessors一个机会来返回代理来替代真正的实例
			Object bean = resolveBeforeInstantiation(beanName, mbdToUse);
			//经过此步骤之后,如果创建了代理或者重写了后置处理器方法并在方法中改变了bean,则直接返回;否则进行常规bean创建
			if (bean != null) {
			    //Aop就是基于这里判断返回的
				return bean;
			}
	...
	protected Object doCreateBean(final String beanName, final RootBeanDefinition mbd, final Object[] args)
		...
		//是否需要提前曝光:单例&允许依赖&当前bean正在创建中,检测循环依赖
		...
		if (earlySingletonExposure) {
			...
			addSingletonFactory(beanName, new ObjectFactory<Object>() {
				...
				public Object getObject() throws BeansException {
					//对bean再一次依赖引用,主要应用SmartInstantiationAware 	BeanPostProcessor,其中Aop就是这里将advice动态织入bean中,
					// 若没有则直接返回bean,不做任何处理
	...
	protected Object resolveBeforeInstantiation(String beanName, RootBeanDefinition mbd) {
        ...
        if (!Boolean.FALSE.equals(mbd.beforeInstantiationResolved)) {
			...
			if (!mbd.isSynthetic() && hasInstantiationAwareBeanPostProcessors()) {
				...
				if (targetType != null) {
				    //实例化前的后置处理器调用,给程序一个修改BeanDefinition的机会
                    //经过处理后可能是一个通过cglib或者其他技术生成的
	...
	protected BeanWrapper createBeanInstance(String beanName, RootBeanDefinition mbd, Object[] args) {
		...
		if (mbd.getFactoryMethodName() != null)  {
			//如果存在工厂方法则使用工厂方法进行初始化:属性factory-method
		...
		//如果已经解析过,则从缓存中获取解析好的构造函数不需要再次确定
		if (resolved) {
	...
	protected void populateBean(String beanName, RootBeanDefinition mbd, BeanWrapper bw) {
		...
		if (hasInstAwareBpps || needsDepCheck) {
			...
			if (hasInstAwareBpps) {
				for (BeanPostProcessor bp : getBeanPostProcessors()) {
					//遍历BeanPostProcessor对InstantiationAwareBeanPostProcessor进行特殊处理
					if (bp instanceof InstantiationAwareBeanPostProcessor) {
						InstantiationAwareBeanPostProcessor ibp = (InstantiationAwareBeanPostProcessor) bp;
		...
		for (String propertyName : propertyNames) {
			try {
				...
				if (Object.class != pd.getPropertyType()) {
					...
					//如:@Autowired List<A> list，将会找到所有匹配A类型的bean并将其注入
	...
	protected void invokeInitMethods(String beanName, final Object bean, RootBeanDefinition mbd)
			throws Throwable {
        //首先检查该bean是否是InitializingBean,如果是的话需要调用afterPropertiesSet
		...
		if (isInitializingBean && (mbd == null || !mbd.isExternallyManagedInitMethod("afterPropertiesSet"))) {
			...
			if (System.getSecurityManager() != null) {
				...
				//属性初始化后的处理调用afterPropertiesSet方法
				((InitializingBean) bean).afterPropertiesSet();
			}
		}
